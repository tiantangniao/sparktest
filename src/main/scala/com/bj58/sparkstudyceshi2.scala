...
object sparkstudyceshi2 {
  ...
  def main(args: Array[String]): Unit = {
    val (zkQuorum, groupId, topics, numThreads) = (
      ...
      "hdp_lbg_ectech_lm_ods_click,hdp_lbg_ectech_lm_ods_search,hdp_lbg_ectech_jzcpc_click",
      1)
    ...
    val ssc = new StreamingContext(new SparkConf().set("spark.kryoserializer.buffer.max", "1024m").setAppName("sparkstudy"), Seconds(60))
    ...
    val sidgrouplines = searchfenkailines.map(value => {
      ...
      if (strarray.length == 34) {
        (strarray(5) + strarray(28), List(value))
      } else if (strarray.length == 24) {
        (strarray(12) + strarray(16), List(value))
      } else if (strarray.length == 53) {
        (strarray(22) + strarray(18), List(value))
      } else {
        ("-", List("-"))
      }
    })
    ...
    //使用window
    val midlledetail = sidgrouplines.reduceByKeyAndWindow((c1: List[String], c2: List[String]) => c1 ::: c2, Seconds(360), Seconds(60))
    ...
    detaillogshandle.foreachRDD(rdditem => {
      if (!rdditem.isEmpty()) {
        ...
        val shuchurdd = rdditem.filter(line => {
          val currenttimelong = line._1.toLong
          (currenttimelong >= minlong && currenttimelong <= middle)
        })
        val seaechcount = shuchurdd.count()

        val clickdianjirdd = shuchurdd.filter(line => {
          val logstr = line._2
          val logvalue = logstr.split("\001")
          ! (logvalue(34).equals("-")&&logvalue(36).equals("-"))
        })
        val clickcount = clickdianjirdd.count()

        shuchurdd.saveAsTextFile("/home/hdp_lbg_ectech/middata/dingxiao/spark/ceshi_7/" + time.toString("yyyy_MM_dd_HH_mm_ss") + "_" + min.toString + "_" + max.toString + "_" + middle.toString + "_" + seaechcount + "_" + clickcount)
        shuchurdd.foreachPartition(x => {
          x.foreach(record => {
